...
def news(request):
    ...
    breaking_posts = News.objects.filter(tags__name="பரபரப்பு").order_by('-published_date')[:8]
    tags_with_news = []
    for tag in tags:
        related_news = News.objects.filter(tags=tag).order_by('-published_date')
        tags_with_news.append((tag, related_news))
    context = {'news': news,'tags':tags, 'bnews':breaking_posts,'tags_with_news': tags_with_news,}
